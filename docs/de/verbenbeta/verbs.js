...
/* =========================================================
   SETS + SEARCH RENDER HOOK
   ========================================================= */

const originalRenderTable =
    renderTable;
...
/*
 * Return the set assigned to a verb.
 */

function getVerbSet(row) {
...
function getVerbInfinitive(row) {
...
/*
 * Return the rows that should actually appear
 * after applying search.
 *
 * This works on the existing displayed rows,
 * so all existing filters and Lucky Mode
...
/*
 * Create a set heading row.
 */

function createSetHeadingRow(
    setName,
    columnCount
) {
...
/*
 * Apply search + sets to the already-rendered
 * table.
 */

function applySetsAndSearch() {
    ...
    /*
     * Remove any previous feature rows.
     */

    tableBody
        .querySelectorAll(
            '.set-heading-row, .search-empty-row'
        )
        .forEach(
            row =>
                row.remove()
    ...
    /*
     * Get the currently displayed rows from
     * the existing filtering/Lucky system.
     */
    ...
    /*
     * Existing normal table rows.
     *
     * Because renderTable() has already rendered
     * the same displayedRows, the DOM rows are
     * currently in exactly the same order.
     */
    ...
    /*
     * Remove all normal rows temporarily.
     */
    ...
    /*
     * Map each displayed row to its original
     * table <tr>.
     */
     ...
     * If Sets are OFF:
     *
     * preserve normal table order,
     * only removing rows that don't match
     * the search.
     */
     ...
     * Sets ON.
     *
     * Group rows by their set while preserving
     * the order in which each set first appears.
     */
    ...
    /*
     * We need the original first-appearance
     * ordering of sets and standalone verbs.
     *
     * Build an ordered structure from the
     * filtered rows.
     */
    ...
    /*
     * Rebuild the table body in grouped order.
     */
...
/*
 * Update the visible # column without
 * changing the underlying data.
 */

function updateDisplayedRowNumber(
    tr,
    number
) {
